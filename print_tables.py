...
Usage:
    python print_tables.py logs/profile_h200nvl_20260507_071801.json
    python print_tables.py --latex logs/profile_h200nvl_20260507_071801.json
"""
...
def print_rich(target_map, draft_map, ctx_lens, block_sizes):
    ...
    # PTD / PTV percentage (paper appendix style)
    table_p = Table(title="PTD / PTV (%) -- per-token drafting cost / per-token verification cost")
...
def print_latex(target_map, draft_map, ctx_lens, block_sizes, source_path: str):
    ...
    print(r"    \centering")
    print(r"    \caption{Per-token drafting cost as a fraction of per-token verification cost, $r(ctx, bs) = \mathrm{PTD}/\mathrm{PTV}$. Lower is better.}")
    print(r"    \label{tab:ptd-ratio}")
    print(r"    \small")
    cols = "r|" + "r" * len(block_sizes)
    print(r"    \begin{tabular}{" + cols + r"}")
    print(r"        \toprule")
    header = " & ".join([r"$ctx$"] + [rf"$bs{{=}}{bs}$" for bs in block_sizes])
    print("        " + header + r" \\")
    print(r"        \midrule")
    for ctx in ctx_lens:
        cells = [str(ctx)]
